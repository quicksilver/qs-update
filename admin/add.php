...
if (@$_POST['submit'] == "New") {
	...
	if (!$plugin->create())
		http_error(400, "Failed creation of plugin");
	/* Move our uploaded files to their final location */
	$plugin_path = $plugin->plugin_file("qspkg", false);
	$info_plist_path = $plugin->plugin_file("qsinfo", false);
	/* TODO: What about the image file ? */
	if (!move_uploaded_file($archive_file["tmp_name"], $plugin_path))
		http_error(400, "Can't move " . $archive_file["tmp_name"] . " to " . $plugin_path);
	if (!move_uploaded_file($info_file["tmp_name"], $info_plist_path))
		http_error(400, "Can't move " . $info_file["tmp_name"] . " to " . $info_plist_path);
	http_error(200, "OK");
}
?>
<html>
